...
- Added current `larena/access` DNA compliance guidance: status is `L4 partial`; decision-layer, grants/context, security/operations and demonstrator baselines are implemented, with installed-site browser smoke as the next safe package-completion step.
