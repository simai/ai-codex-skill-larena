...
- Added current `larena/access` DNA compliance guidance: status is `L3 partial`, and the next safe implementation layer is `AccessValue` / `AccessContext` / `AccessDecision` before table migrations or broad API/AI exposure.
