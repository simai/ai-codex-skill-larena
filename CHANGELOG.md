...
- Added current `larena/access` DNA compliance guidance: status is `L3+ partial`; `AccessValue` / `AccessContext` / `AccessDecision` are implemented, and the next safe layer is grants/context plus token scopes/audit before broad API/AI exposure.
