...
- When asked to "check package against standard" / "проверь пакет на стандарт", run a full package-completion audit: inspect the package repository, compare it with `simai/larena` standards under `docs/developer/standards/`, return an artifact matrix, current `L0`-`L5`, blockers, verification evidence and the next batch required to raise the package one level.
- During the current Larena package-standardization phase, treat `L4 Demonstration Ready` as the target level. Mark `L5 Release / Marketplace Ready` as deferred until the Laravel update-server/distribution standard defines module artifacts, package data packing, delivery channels, signatures/checksums, update plans and rollback policy.
...
- After updating an installed `larena/*` package in a local Larena site, run `php artisan migrate --force` and the idempotent `php artisan simai:install` before browser smoke. New package operations, menu links and access metadata can exist in config/module files but still be absent from the runtime database until the installer syncs them.
